...
###Please refer to Extended.Data.Fig.2c.R for generation of "LungVNG" Seurat object

LungVNG@meta.data$LungNvsP <- "n"
Lung_Npy2r <- subset(LungVNG, Npy2r > 0.1)
Lung_P2ry1 <- subset(LungVNG, P2ry1 > 0.1)
LungVNG@meta.data[rownames(Lung_Npy2r@meta.data),]$LungNvsP <- 'Lung_Npy2r'
LungVNG@meta.data[rownames(Lung_P2ry1@meta.data),]$LungNvsP <- 'Lung_P2ry1'

genelist <- c("Calca", "Calcb", "Vip", "Tac1")

#ED. Fig. 7a: heatmap
DoHeatmap(subset(LungVNG, subset = LungNvsP !="n"), group.by = "LungNvsP", features = genelist)

#ED. Fig. 7b: violin plot
cell1 <- colnames(Lung_Npy2r)
length(cell1)
cell2 <- colnames(Lung_P2ry1)
length(cell2)
data <- LungVNG@assays$RNA@data
data[1:5,1:5]
...
library(ggpubr)
p1 <- ggplot(datAll,aes(type,Expression_level,fill=type))+
  geom_violin()+
...
ggsave(
  filename = "lungVSN_neuropeptide_vln.svg",
